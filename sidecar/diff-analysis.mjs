...
import { coerceStructuredReviewShape, hasStructuredReviewShape } from './review-shape.mjs';
import { REVIEW_RESPONSE_SCHEMA, REVIEW_TOOL_SCHEMA } from './diff-analysis-schema.mjs';
import { normalizeReviewResult } from './diff-analysis-normalize.mjs';
...
export async function analyzeDiff(runtime, payload) {
	...
	try {
		...
		let structuredResult = readStructuredReviewResult(
			capturedResult,
			session.getLastAssistantText()
		);
		if (!structuredResult) {
			capturedResult = null;
			await session.prompt(buildRepairPrompt(payload, session.getLastAssistantText()));
			structuredResult = readStructuredReviewResult(capturedResult, session.getLastAssistantText());
		}
		if (!structuredResult) {
...
function readStructuredReviewResult(capturedResult, assistantText) {
	if (capturedResult) {
		return hasStructuredReviewShape(capturedResult) ? capturedResult : null;
	}
	return extractStructuredReviewResult(assistantText);
}
...
function buildRepairPrompt(payload, previousText) {
	return [
		'Your previous diff review was not machine-readable.',
		'Return JSON only. Do not include Markdown, prose, or code fences.',
		'The JSON object must match this exact top-level shape:',
		JSON.stringify(reviewResultJsonExample(payload.diff.files), null, 2),
		'',
		'Convert the previous response into that shape if possible. If a section has no grounded items, use an empty array.',
		'Every focusQueue item must use a hunkId from the diff payload.',
		'',
		'Previous response:',
		truncateText(previousText, 4_000),
		'',
		'Diff payload:',
		JSON.stringify(compactDiffPayload(payload.diff), null, 2)
	].join('\n');
